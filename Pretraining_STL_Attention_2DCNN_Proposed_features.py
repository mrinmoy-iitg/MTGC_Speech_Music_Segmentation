...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  9 17:55:22 2022
...
def get_preinitialized_model(pretrain_model, n_fc_nodes, n_fc_lyrs):
    # pretrain_model.trainable = False
    pretrained_layers = pretrain_model.layers[-2].output

    ''' Fully Connected layers '''
    x = Dense(n_fc_nodes)(pretrained_layers)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = Dropout(0.1)(x)
    ...
    output_layer = Dense(len(PARAMS['genre_list']), activation='sigmoid')(x)        
    model = Model(pretrain_model.inputs, output_layer)
...
def generator(PARAMS, file_list, batchSize, add_noise=False, PRE_TRAINING=False):
    batch_count = 0
    ...
    while 1:    
        ...
        if PRE_TRAINING:
            batchLabel_new = np.zeros((np.shape(batchLabel)[0],2))
            genre_label_list = {PARAMS['genre_list'][genre_i]:genre_i for genre_i in PARAMS['genre_list'].keys()}
            for batch_i in range(batchSize):
                genre_lab = np.where(batchLabel[batch_i,:]==1)[0].tolist()
                pretrain_music_lab = [1 if genre_label_list[lab] in PARAMS['spoken_genres'] else 0 for lab in genre_lab]
                music_label = np.mean(pretrain_music_lab)

                pretrain_minority_lab = [1 if genre_label_list[lab] in PARAMS['minority_genres'] else 0 for lab in genre_lab]
                minority_label = np.mean(pretrain_minority_lab)

                batchLabel_new[batch_i,:] = [music_label, minority_label]
            batchLabel = batchLabel_new.copy()
            # batchLabel = np.array(batchLabel, ndmin=2).T
            # print(f"PRE_TRAINING batchLabel: {np.shape(batchLabel)} {np.shape(train_input['cbow_feat_input'])}")
...
def model_pretraining(PARAMS, model, TR_STEPS, weightFile, logFile):
    csv_logger = CSVLogger(logFile, append=True)
    weightFile = weightFile.replace('.h5','_pretrain.h5')
    logFile = logFile.replace('.csv','_pretrain.csv')
    ...
    # Train the model
    print('SPE: ', TR_STEPS, ' VSteps: ', PARAMS['V_STEPS'])

    model.fit(
        generator(PARAMS, PARAMS['train_files'], PARAMS['batch_size'], add_noise=PARAMS['add_noise_data_augmentation'], PRE_TRAINING=True),
        steps_per_epoch=TR_STEPS,
        epochs=PARAMS['epochs'],
        verbose=1,
        validation_data=generator(PARAMS, PARAMS['val_files'], PARAMS['batch_size'], PRE_TRAINING=True),
...
def model_training(PARAMS, model, TR_STEPS, weightFile, logFile):
    csv_logger = CSVLogger(logFile, append=True)
    mcp = ModelCheckpoint(weightFile, monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=True, mode='min', save_freq='epoch')
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, restore_best_weights=True, min_delta=0.001, patience=10)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=0.00000001, verbose=1, mode='min', min_delta=0.01)
    ...
    model.fit(
        generator(PARAMS, PARAMS['train_files'], PARAMS['batch_size'], add_noise=PARAMS['add_noise_data_augmentation'], PRE_TRAINING=False),
        steps_per_epoch=TR_STEPS,
        epochs=PARAMS['epochs'],
        verbose=1,
        validation_data=generator(PARAMS, PARAMS['val_files'], PARAMS['batch_size'], PRE_TRAINING=False),
        validation_steps=PARAMS['V_STEPS'],
        callbacks=[csv_logger, es, mcp, reduce_lr],
        )
...
def perform_training(PARAMS):
    ...
    if not os.path.exists(weightFile.replace('.h5','_pretrain.h5')):
        pretrain_model, learning_rate = cnn_model(PARAMS)
        misc.print_model_summary(summaryFile.replace('.txt','_pretrain.txt'), pretrain_model)
        print(pretrain_model.summary())
        print('Pre-traning: ACNN architecture with time-feature attention')
        
        pretrain_model, trainingTimeTaken = model_pretraining(PARAMS, pretrain_model, PARAMS['TR_STEPS']*PARAMS['W_shift']/10000, weightFile, logFile)
    else:
        print('Pre-traning: ACNN architecture with time-feature attention. EXISTS!')
        pretrain_model, learning_rate = cnn_model(PARAMS)
        pretrain_model.load_weights(weightFile.replace('.h5','_pretrain.h5'))
        

    if not os.path.exists(paramFile):        
        # pretrained_layers = pretrain_model.layers[-2].output
        # output_layer = Dense(len(PARAMS['genre_list']), activation='sigmoid')(pretrained_layers)        
        # model = Model(pretrain_model.inputs, output_layer)
        # opt = optimizers.Adam(lr=learning_rate)
        # model.compile(loss='binary_crossentropy', optimizer=opt, metrics=[AUC(curve='PR')])
        
        model = get_preinitialized_model(pretrain_model, n_fc_nodes=512, n_fc_lyrs=3)
        
        misc.print_model_summary(summaryFile, model)
        print(model.summary())
        print('Fine tuning: ACNN architecture with time-feature attention')

        model, trainingTimeTaken = model_training(PARAMS, model, PARAMS['TR_STEPS'], weightFile, logFile)
        
        if PARAMS['save_flag']:
            np.savez(paramFile, lr=str(learning_rate), TTT=str(trainingTimeTaken))

    model, learning_rate = cnn_model(PARAMS)
    # output_layer = Dense(len(PARAMS['genre_list']), activation='sigmoid')(model.layers[-2].output)        
    # model = Model(model.inputs, output_layer)
    # opt = optimizers.Adam(lr=learning_rate)
    # model.compile(loss='binary_crossentropy', optimizer=opt, metrics=[AUC(curve='PR')])
    model = get_preinitialized_model(model, n_fc_nodes=512, n_fc_lyrs=3)
...
def __init__():
    PARAMS = {
            ...
            'musical_genres': ['Action', 'Fantasy', 'Sci-Fi', 'Thriller', 'Animation', 'Mystery', 'Horror'],
            'spoken_genres': ['Romance', 'Comedy', 'Family', 'Drama', 'Crime', 'Biography'],
            }
